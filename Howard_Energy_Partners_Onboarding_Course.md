<!--
author:   Howard Energy Partners — People & Culture
email:    jimpyle@gmail.com
version:  1.1.0
language: en
narrator: US English Male

comment:  An interactive onboarding course introducing new employees to Howard Energy Partners — our history, our midstream business, key milestones, innovation, and culture. Includes self-check quizzes and a final knowledge check.

link:     https://cdnjs.cloudflare.com/ajax/libs/animate.css/4.1.1/animate.min.css
-->

# Howard Energy Partners: New Employee Onboarding Demo - not for official use.

--{{0}}--
Thank you for viewing this demo prepared by Jim Pyle 210-867-1284

<!-- class="animate__animated animate__fadeInDown" -->
**Welcome to the team!**

This course introduces you to **who we are**, **what we do**, and **why it matters**. By the end, you'll understand our history, our business, our biggest milestones, and the culture you're now part of.

> ⏱️ **Estimated time:** 25–35 minutes
> 🎯 **Audience:** New employees / onboarding
> 📋 **How to use it:** Read each module, then answer the **Quick Check** before moving on. A full **Knowledge Check** waits at the end.

> 💡 **Tip:** Use the mode switcher (top of the page) to try **Presentation mode** — the course will read itself aloud.

## Learning Objectives

--{{0}}--
Here is what you should be able to do after finishing this course.

By the end of this course, you will be able to:

1. Describe how Howard Energy Partners was founded and how it is structured.
2. Explain what "midstream energy" means and where Howard Energy Partners operates.
3. Recall the company's major growth milestones.
4. Understand HEP's role in energy innovation and lower-carbon solutions.
5. Connect day-to-day work to the company's purpose and core values.

- [ ] I'm ready to begin

## Module 1 — Company Snapshot

--{{0}}--
Let's start with the basics: who we are, where we're based, and who leads us.

Howard Energy Partners (HEP) is a privately held **midstream energy and logistics** company headquartered in **San Antonio, Texas**. The company was **founded in June 2011** by **Mike Howard**, who still serves as **Chairman and CEO** today.[^1][^2]

Howard Energy Partners describes itself as *"part of the industry that powers every other industry."* It doesn't drill for oil and gas, and it doesn't sell it to consumers — instead, it builds and operates the critical infrastructure that moves, processes, and prepares energy as it travels from the wellhead to the market.[^3]

What makes Howard Energy Partners's ownership unusual
==================================

--{{1}}--
Pay attention to this part — our ownership structure is a big reason we can think long term.

     {{1}}
Unlike most companies its size, Howard Energy Partners is backed by long-term institutional partners — the **Alberta Investment Management Corporation (AIMCo)** and **Alinda Capital Partners** — rather than traditional private equity firms. This structure is intentional: it lets the company take a **long-term view** and grow at the pace its customers need, rather than chasing short-term returns.[^4]

     {{2}}
Today, Howard Energy Partners is one of the nation's **largest private energy infrastructure companies** and a **multi-billion-dollar enterprise**, with operations across **Texas, New Mexico, Pennsylvania, Oklahoma, Kansas, and Mexico**.[^2][^5]

Meet the leader
===============

CEO **Mike Howard** holds a degree in chemical engineering from **Texas A&M University–Kingsville** and previously served as **President of Midstream** for a major Fortune 100 energy company before founding HEP. In **February 2026**, he was appointed to the **National Petroleum Council**, a federal advisory committee created in 1946 at the request of President Harry S. Truman.[^5][^6]

> 💡 **Did You Know?** The company name reflects a personal idea: founder Mike Howard wanted to build a *"meaningful endeavor"* — a values-driven company that delivers reliable energy while contributing to its communities. That phrase still shapes how Howard Energy Partners makes growth decisions today.[^3]

### Quick Check — Module 1

In what **year** was Howard Energy Partners founded?

[[2011]]
[[?]] It's the same decade smartphones went mainstream.
[[?]] Early 2010s.
***
Howard Energy Partners was founded in **June 2011** in San Antonio, Texas.
***

Howard Energy Partners is backed by **traditional private equity firms**.

[( )] True
[(X)] False
[[?]] Think about the AIMCo and Alinda detail.
***
**False.** Howard Energy Partners is backed by long-term partners — AIMCo and Alinda Capital — *not* traditional private equity. This is what enables its long-term strategy.
***

## Module 2 — What We Do: Understanding Midstream

--{{0}}--
The energy industry has three main parts. Let's see where we fit.

The energy industry is often split into three parts. **Howard Energy Partners lives in the midstream.**

| Segment        | What it does                                       | Example                     |
| :------------- | :------------------------------------------------- | :-------------------------- |
| **Upstream**   | Finds and produces oil & gas                       | Drilling a well             |
| **Midstream**  | Gathers, processes, transports, and stores energy  | Pipelines, plants, terminals|
| **Downstream** | Refines and sells to end users                     | Refineries, gas stations    |

Howard Energy Partners provides producers with a full suite of integrated services "from wellhead to market" — natural gas gathering, processing, treating, transportation, fractionation, terminals, and rail and marine logistics.[^3][^4]

Where Howard Energy Partners operates
===================

--{{1}}--
Our footprint is organized into regions, each serving a different need.

     {{1}}
- **South Texas** — Processes and transports natural gas from Webb County (Eagle Ford Shale) to Gulf Coast markets and Mexico, including supplying growing LNG export facilities.
- **Mexico** — The **200-mile Nueva Era Pipeline** carries gas to northern Mexico (Monterrey) via a joint venture with **Grupo Clisa**; the gas can be up to **50% less carbon-intensive** than the fuels it displaces.
- **Pennsylvania** — A large gathering system in the prolific **Marcellus Shale**.
- **West Texas** — With **Devon Energy**, large-scale gathering and cryogenic processing in the **Delaware Basin**, helping reduce flaring.
- **Port Arthur** — A first-of-its-kind terminal handling motor fuels, petrochemical feedstocks, and **lower-carbon fuels** like renewable diesel and sustainable aviation fuel (SAF).
- **Corpus Christi** — Home to the innovative **Javelina** plant, plus hydrogen sales and rail/marine logistics.
- **Oklahoma–Kansas** — The **Midship Pipeline** and the newly acquired **Superior Midstream** gathering system anchor HEP's growing Midcontinent presence.

> 💡 **Did You Know?** Renewable diesel and SAF are **100% compatible** with existing engines and infrastructure — no new equipment needed — and can cut greenhouse gas emissions by up to **80%**.[^7]

### Quick Check — Module 2

Which energy segment does Howard Energy Partners operate in?

[( )] Upstream
[(X)] Midstream
[( )] Downstream
***
Howard Energy Partners is a **midstream** company — it connects producers to markets through gathering, processing, storage, and logistics.
***

Match each region to its description:

[[ (South Texas) | Mexico | Pennsylvania ]] feeds growing Gulf Coast LNG export facilities, and the [[ South Texas | (Mexico) | Pennsylvania ]] business runs the cross-border Nueva Era Pipeline, while [[ South Texas | Mexico | (Pennsylvania) ]] hosts a gathering system in the Marcellus Shale.

## Module 3 — Our Growth Story: Milestones

--{{0}}--
Howard Energy Partners grew from a 2011 startup into a multi-billion-dollar enterprise through a deliberate mix of organic growth and acquisitions. Here are the headline milestones.

| Year         | Milestone                                                                                   |
| :----------- | :------------------------------------------------------------------------------------------ |
| **2011**     | Howard Energy Partners founded in San Antonio, TX.[^1]                                       |
| **2021**     | Acquired the **Javelina** treating & fractionation facility in Corpus Christi (from MPLX).[^8]|
| **2023**     | Adjacent **Infinium Pathfinder** eFuels facility — fed by CO₂ from Javelina — began production.[^9]|
| **2024**     | A **record M&A year** with transactions totaling **more than $1.1 billion**.[^10]            |
| **Jan 2025** | Acquired a **120-mile ethylene pipeline** from EPIC Midstream Holdings.[^10]                 |
| **Feb 2025** | Acquired an operating interest in the **Midship Pipeline** (1.1 Bcf/d) from a Cheniere subsidiary.[^11]|
| **Aug 2025** | Priced a **$750 million senior notes offering** (6.625%, due 2034).[^12]                     |
| **May 2026** | Closed acquisition of **Superior Midstream** (~**3,700 miles** of gathering pipeline).[^13]  |
| **May 2026** | Increased ownership of the **Midship Pipeline to 100%**.[^13]                                |
| **Feb 2026** | CEO Mike Howard appointed to the **National Petroleum Council**.[^6]                         |

> 💡 **Did You Know?** After taking over operations of the Midship Pipeline, Howard Energy Partners optimized the asset so effectively that it achieved **record throughput and utilization** — a great example of "operational excellence" in action.[^13]

### Quick Check — Module 3

What made **2024** a record year for HEP?

[( )] It opened its first international office
[(X)] It completed over $1.1 billion in M&A transactions
[( )] It went public on the NYSE
[( )] It hired its 10,000th employee
***
2024 was a record year for **M&A activity**, with several material transactions totaling **more than $1.1 billion**.
***

Roughly how many miles of gathering pipeline did the **2026 Superior Midstream** acquisition add?

[( )] ~200 miles
[( )] ~1,500 miles
[(X)] ~3,700 miles
[( )] ~10,000 miles
***
The Superior Midstream acquisition added approximately **3,700 miles** of gathering pipeline across Oklahoma, Kansas, the Texas Panhandle, and East Texas.
***

## Module 4 — Innovation & Lower-Carbon Energy

--{{0}}--
Our strategy isn't just about moving traditional fuels. Many of our facilities also address lower-carbon demand. The flagship example is the Javelina plant.

HEP's facilities are designed to **solve today's energy needs while also addressing lower-carbon demand**.[^3]

The flagship example is the **Javelina** plant in Corpus Christi. Javelina captures **waste gas** from local refineries — emissions that would otherwise be discarded — and turns them into valuable products like hydrogen, ethylene, and natural gas liquids. This is **emissions recycling**: it reduces emissions *and* creates value from waste.[^7][^8]

     {{1}}
Javelina also became the foundation for a landmark clean-energy partnership. The discarded **CO₂** from Javelina is captured and supplied to neighboring clean-fuels innovator **Infinium**, whose **Project Pathfinder** facility combines that CO₂ with green hydrogen to produce **net-zero electro-fuels (eFuels)** — a drop-in replacement for diesel used to help decarbonize heavy transport. This helped make Javelina **among the first clean hydrogen facilities along the U.S. Gulf Coast**.[^7][^9]

> 💡 **Did You Know?** In 2024, **Bill Gates** and his Breakthrough Energy team toured the Infinium eFuels facility located at HEP's Javelina Plant — describing it as a groundbreaking demonstration of clean-fuel technology. Not every day a midstream plant draws a visit from one of the world's most famous innovators![^14][^9]

How the Javelina + Infinium loop works
======================================

```ascii  Fig.: Turning refinery waste into clean fuel
  +-------------------+      waste gas      +-------------------+
  | Local Refineries  | ------------------> |  Howard Energy Partners"Javelina"   |
  +-------------------+                     |  recycling plant  |
                                            +-------------------+
                                              |            |
                              H2, ethylene,   |            |  captured CO2
                              NGLs (products) |            v
                                              v        +-------------------+
                                          (to market)  | Infinium eFuels   |
                                                        | (Project Pathfinder)|
                                                        +-------------------+
                                                                 |
                                                                 v
                                                        net-zero e-diesel
```

### Quick Check — Module 4

What does the **Javelina** plant do with refinery waste gas, and who is its clean-fuels partner?

[(X)] Recycles it into useful products and supplies captured CO₂ to Infinium for net-zero eFuels
[( )] Burns it off as flaring
[( )] Ships it overseas for disposal
[( )] Stores it underground indefinitely
***
Javelina **recycles** the waste gas into hydrogen, ethylene, and NGLs, and supplies captured **CO₂ to Infinium** to make net-zero eFuels.
***

## Module 5 — Howard Energy By the Numbers

--{{0}}--
Here is a snapshot of HEP's scale, and how quickly it has grown.

A snapshot of HEP's scale (as reported in early 2025):[^10]

- 🚧 **1,500+ miles** of pipeline (U.S. and Mexico)
- ⚙️ **~1 billion cubic feet/day** of cryogenic processing capacity
- 🛢️ **~2.5 million barrels** of tank storage capacity
- 🌎 Operations across **4 U.S. states + Mexico**
- 🤝 **150+ years** of combined leadership experience on the executive team[^4]

The chart below illustrates HEP's rapid pipeline growth (figures are approximate and grow as the company expands):

<!-- data-type="BarChart" data-title="Approx. Pipeline Miles Over Time" data-ylabel="Miles of pipeline" data-show -->
| Milestone          | Pipeline Miles |
| :----------------- | -------------: |
| Founded (2011)     |              0 |
| Early 2025         |           1500 |
| After 2026 deals   |           5200 |

> 🏆 **Recognition:** Howard Energy Partners has been honored as a **USA TODAY Top Workplace**, reflecting its emphasis on culture and employee experience.[^3]

## Module 6 — Our Culture & Core Values

--{{0}}--
Finally, the culture you're now part of. Our mission is captured in a simple phrase you'll hear often: delivering positive energy.

HEP's mission is captured in a phrase you'll hear often: **"delivering positive energy."** It refers both to the reliable energy the company provides *and* to the kind of workplace it tries to be.[^3]

The company's **core values were created by employees themselves.** The consistent themes are:

- **Safety** and **operational excellence**
- **Innovation**
- **Teamwork, honesty, and doing what is right**
- A long-term, **purpose-driven** approach to growth[^1][^3]

Howard Energy Partners frames its work as a **"meaningful endeavor"** — the belief that access to reliable, affordable energy is foundational to human progress and quality of life.[^3][^5]

> 💡 **Did You Know?** Howard Energy Partners teams are active in their communities — for example, partnering with local organizations in Corpus Christi to deliver school supplies to first-grade students. "Delivering positive energy" extends beyond pipelines.[^14]

### Quick Check — Module 6

HEP's guiding purpose is expressed in three words. Complete the phrase: "Delivering positive [[energy]]."

[[?]] It's also the name of the industry you now work in.
***
The guiding purpose is **"delivering positive energy"** — both the product we provide and the culture we build.
***

## 🧠 Final Knowledge Check

--{{0}}--
Time to put it all together. Answer each question — hints and explanations are available if you need them.

**1.** In what city is Howard Energy Partners headquartered?

[[San Antonio]]
[[?]] It's in Texas.
***
Howard Energy Partners is headquartered in **San Antonio, Texas**.
***

**2.** Which of these are **Howard Energy Partners operating regions**? (Select all that apply.)

[[X]] South Texas
[[X]] Pennsylvania
[[X]] Oklahoma–Kansas
[[ ]] Alaska
[[X]] Mexico
[[ ]] Florida
***
Howard Energy Partners operates across South Texas, West Texas, Pennsylvania, Port Arthur, Corpus Christi, Oklahoma–Kansas, and Mexico — but **not** Alaska or Florida.
***

**3.** What is the name of the 200-mile cross-border pipeline that delivers natural gas to northern Mexico?

[[ Nueva Era | Midship | Javelina ]]
***
The **Nueva Era Pipeline** — a joint venture with Grupo Clisa — supplies gas to industrial northern Mexico.
***

**4.** Match the milestone to its outcome:

[[Record M&A] [Midship] [Superior Midstream]]
[    (X)         ( )         ( )            ] $1.1B+ of transactions in a single year
[    ( )         (X)         ( )            ] Howard Energy Partners increased ownership to 100% in 2026
[    ( )         ( )         (X)            ] Added ~3,700 miles of gathering pipeline

**5.** Who is HEP's founder and CEO, and what 2026 federal appointment did he receive?

[(X)] Mike Howard — appointed to the National Petroleum Council
[( )] Mike Howard — appointed U.S. Secretary of Energy
[( )] Brad Bynum — appointed to the Federal Reserve
[( )] Mike Howard — appointed to the Supreme Court
***
**Mike Howard**, founder, Chairman, and CEO, was appointed to the **National Petroleum Council** in February 2026.
***

Great work — you've completed the knowledge check!

- [ ] I completed the Final Knowledge Check

## 💬 Feedback Survey

--{{0}}--
Your feedback helps us improve onboarding. These questions have no right answer.

How confident do you feel about HEP's business after this course?

- [(very)]     Very confident
- [(somewhat)] Somewhat confident
- [(neutral)]  Neutral
- [(not)]      Not yet — I'd like more detail

Which module was most useful to you?

    [[Company Snapshot]]
    [[Understanding Midstream]]
    [[Milestones]]
    [[Innovation]]
    [[By the Numbers]]
    [[Culture & Values]]

Anything you'd like to learn more about?  Call Jim Pyle @ 210-867-1284.

    [[___ ___ ___]]

## 🎓 Certificate of Completion

--{{0}}--
Congratulations on completing the Howard Energy Partners New Employee Onboarding course! Enter your name below and print your certificate.

**Congratulations on completing the course!**

Enter your full name to generate your certificate, then click **Print Certificate**.

<lia-keep>
<div style="margin: 20px 0;">
  <input type="text" id="recipientName" placeholder="Enter your full name here"
    style="padding: 10px 14px; font-size: 18px; width: 340px; border: 2px solid #003087; border-radius: 6px; outline: none;"
    oninput="document.getElementById('certName').textContent = this.value || 'Your Name'" />
</div>

<div id="certificate" style="border: 10px double #003087; padding: 50px 60px; max-width: 680px; margin: 30px auto; text-align: center; background: #fff; font-family: Georgia, serif; box-shadow: 0 4px 16px rgba(0,0,0,0.12);">
  <div style="color: #003087; font-size: 13px; letter-spacing: 4px; text-transform: uppercase; margin-bottom: 6px;">Howard Energy Partners</div>
  <div style="font-size: 11px; color: #888; letter-spacing: 2px; text-transform: uppercase; margin-bottom: 24px;">People &amp; Culture</div>
  <div style="font-size: 30px; font-weight: bold; color: #003087; margin-bottom: 28px; border-bottom: 2px solid #c8a951; padding-bottom: 16px;">Certificate of Completion</div>
  <div style="font-size: 15px; color: #555; margin-bottom: 8px;">This certifies that</div>
  <div id="certName" style="font-size: 34px; color: #003087; font-style: italic; border-bottom: 2px solid #003087; padding-bottom: 8px; margin: 8px 40px 20px; min-height: 48px;">Your Name</div>
  <div style="font-size: 15px; color: #555; margin-bottom: 6px;">has successfully completed the</div>
  <div style="font-size: 19px; font-weight: bold; color: #222; margin-bottom: 6px;">New Employee Onboarding Course</div>
  <div style="font-size: 14px; color: #777; margin-bottom: 36px;">Howard Energy Partners — People &amp; Culture</div>
  <div style="display: flex; justify-content: space-around; margin-top: 30px; padding-top: 20px; border-top: 1px solid #ddd;">
    <div style="text-align: center;">
      <div style="font-size: 13px; color: #444; margin-bottom: 4px;">Mike Howard</div>
      <div style="font-size: 11px; color: #888;">Chairman &amp; CEO</div>
    </div>
    <div style="text-align: center;">
      <div id="certDate" style="font-size: 13px; color: #444; margin-bottom: 4px;"></div>
      <div style="font-size: 11px; color: #888;">Date Completed</div>
    </div>
  </div>
</div>

<button onclick="window.print()"
  style="background: #003087; color: white; padding: 12px 32px; font-size: 16px; border: none; border-radius: 6px; cursor: pointer; margin-top: 10px; letter-spacing: 1px;">
  🖨️ Print Certificate
</button>

<script>
  document.getElementById('certDate').textContent = new Date().toLocaleDateString('en-US', {year:'numeric', month:'long', day:'numeric'});
</script>

<style>
  @media print {
    body > *:not(#certificate) { display: none !important; }
    #certificate { box-shadow: none !important; border: 10px double #003087 !important; }
  }
</style>
</lia-keep>

## 📚 Sources

1. PrivCo / ZoomInfo / LinkedIn — founding year (2011) and San Antonio headquarters. [privco.com](https://www.privco.com/company/howard-midstream-energy)
2. Howard Energy Partners — *Mike Howard bio* (founder, Chairman & CEO; multi-billion-dollar enterprise). [howardenergypartners.com](https://www.howardenergypartners.com/howard-bio/)
3. Howard Energy Partners — *Who We Are* and *Home* pages (purpose, "meaningful endeavor," core values, Top Workplaces). [howardenergypartners.com](https://www.howardenergypartners.com/who-we-are/)
4. Howard Energy Partners — *About* page (AIMCo & Alinda ownership; 150+ years combined experience; wellhead-to-market services). [howardenergypartners.com](https://www.howardenergypartners.com/about)
5. BusinessWire / Yahoo Finance — *Mike Howard Appointed to National Petroleum Council* (Feb 3, 2026). [businesswire.com](https://www.businesswire.com/news/home/20260203292099/en)
6. BusinessWire — National Petroleum Council appointment details. [businesswire.com](https://www.businesswire.com/news/home/20260203292099/en)
7. Howard Energy Partners — *Our Operations* page (regional operations, Nueva Era, Javelina/Infinium, renewable diesel & SAF). [howardenergypartners.com](https://www.howardenergypartners.com/our-operations/)
8. BusinessWire — *HEP to Acquire Refinery Services Facility in Corpus Christi* (Javelina acquisition, 2021). [businesswire.com](https://www.businesswire.com/news/home/20201229005030/en)
9. Hydrogen Insight & Infinium — world's first commercial drop-in eFuels facility (Pathfinder), CO₂ feedstock from HEP. [infiniumco.com](https://www.infiniumco.com/projects)
10. Howard Energy Partners — *Acquires Ethylene Pipeline from EPIC Midstream Holdings* (Jan 2025); record $1.1B M&A year. [howardenergypartners.com](https://www.howardenergypartners.com/howard-energy-partners-acquires-ethylene-pipeline-from-epic-midstream-holdings/)
11. Howard Energy Partners — *Acquires Operating Interest in Midship Pipeline* (Feb 2025). [howardenergypartners.com](https://www.howardenergypartners.com/howard-energy-partners-acquires-operating-interest-in-midship-pipeline/)
12. BusinessWire — *HEP Announces Pricing of $750 Million Senior Notes Offering* (Aug 2025). [businesswire.com](https://www.businesswire.com/news/home/20250213998958/en)
13. BusinessWire / Oil & Gas Journal — *HEP Expands Midcontinent Platform With Strategic Acquisitions* (June 2026). [businesswire.com](https://www.businesswire.com/news/home/20260602356932/en/Howard-Energy-Partners-Expands-Midcontinent-Platform-With-Strategic-Acquisitions)
14. Howard Energy Partners LinkedIn — Bill Gates / Breakthrough Energy tour of the Javelina-adjacent Infinium eFuels facility; community engagement examples. [linkedin.com](https://www.linkedin.com/company/howard-energy-partners)

--{{0}}--
That's the end of the course. Welcome aboard, and thank you for taking the time to learn about Howard Energy Partners.
